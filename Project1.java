...
public class Project1 {
    ...
    private static void calculateAndDisplayTimes(int n, int[]     burstTimes, int[] arrivalTimes) { 
        ...
        float awt = 0;         float art = 0;         float att = 0; 
 
        for (int i = 0; i < n; i++) { 
            System.out.println((i + 1) + "\t  ||\t" + burstTimes[i] + 
"\t||\t" + arrivalTimes[i] + "\t||\t" + wt[i] + "\t||\t " 
                    + rt[i] + "\t||\t " + ct[i]);             awt += wt[i];             art += rt[i]; 
            att += (ct[i] - arrivalTimes[i]); 
        } 
        awt = awt / n;         art = art / n;         att = att / n; 
        System.out.println("\nAverage waiting time = " + awt); 
        System.out.println("\nAverage response time = " + art);         System.out.println("\nAverage turnaround time = " + att); 
    }
    ...
    static void findAvgTime(int processes[], int n, int burstTimes[], int quantum, int arrivalTimes[]) { 
        ...
        double total_wt = 0, total_tat = 0, total_rt = 0;   findWaitingTime(processes, n, burstTimes, wt, quantum, arrivalTimes, ct, rt); 
...
burstTimes[i] + "\t " + wt[i] + "\t\t " + tat[i] + "\t\t " + rt[i]); 
    ...
    static void findWaitingTime(int processes[], int n, int burstTimes[], int wt[], int quantum, int arrivalTimes[],
            int ct[], int rt[]) {
    ...
    static void findTurnAroundTime(int processes[], int n, int burstTimes[], int wt[], int tat[], int ct[],
            int arrivalTimes[]) {
        for (int i = 0; i < n; i++)
    ...
    static void compareAlgorithmsEfficiency(double avgWaitingTimeRR, double avgWaitingTimeFCFS) {
        if (avgWaitingTimeRR < avgWaitingTimeFCFS) {
            System.out.println(
                    "Round Robin (RR) algorithm results in the minimum average waiting time = " + avgWaitingTimeRR);
